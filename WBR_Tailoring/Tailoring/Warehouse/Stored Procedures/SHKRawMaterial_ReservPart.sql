CREATE PROCEDURE [Warehouse].[SHKRawMaterial_ReservPart]
	@spcvc_id INT,
	@employee_id INT,
	@data_xml XML,
	@qty DECIMAL(9, 3)
AS
	...
	DECLARE @qty_need_full DECIMAL(9, 3)
	DECLARE @qty_now_reserv DECIMAL(9, 3)
	DECLARE @rmt_id INT
	DECLARE @okei_id INT
	DECLARE @sp_id INT
	...
	SELECT	@error_text = CASE 
	      	                   ...
	      	                   WHEN  spcvc.consumption * spcv.qty <= 0 THEN 
	      	                        'Количество необходимого материала должно быть больше 0.'
	      	                   WHEN c.no_check_reserv = 0 AND spcvc.consumption * spcv.qty <= ISNULL(oar.qty, 0) THEN 
	      	                        'Текущие резервы уже перекрывают потребность'
	      	                   WHEN c.no_check_reserv = 0 AND  spcvc.consumption * spcv.qty < @qty + ISNULL(oar.qty, 0) THEN 
	      	                        'Пытаетесь зарезервировать больше потребности'
	      	                   ELSE NULL
	      	              END,
			@qty_need     = @qty,
			@qty_res      = @qty,
			@qty_need_full = spcvc.consumption * spcv.qty,
			@qty_now_reserv = ISNULL(oar.qty, 0),
			@rmt_id       = spcvc.rmt_id,			
			@okei_id      = spcvc.okei_id,
			@sp_id        = spcv.sp_id,
			@no_check_reserv = c.no_check_reserv
	...
	IF @no_check_reserv = 0 AND @qty_need > @qty_need_full - @qty_now_reserv
	BEGIN
		RAISERROR('Вы ставете резерв больше потребности', 16, 1)
	    RETURN
	END
	...
	BEGIN TRY
		...
		OUTER APPLY (
			      	SELECT	SUM(smr.quantity) qty
			      	FROM	Warehouse.SHKRawMaterialReserv smr
			      	WHERE	smr.spcvc_id = cvc.spcvc_id
			      ) oar
		WHERE	cvc.spcvc_id = @spcvc_id AND oar.qty >= @qty_need_full
