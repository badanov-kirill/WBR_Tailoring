CREATE PROCEDURE [Products].[ProdArticleForPortal_ByXML]
	@data_xml XML
AS
	SET NOCOUNT ON
	SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED
	
	RAISERROR('Функционал отключен, невозможно вычислить баркод',16,1)
	RETURN
	...
	FROM	@data_xml.nodes('root/det')x(ml)   
			INNER JOIN	Products.Brand b
				ON	b.brand_name = ml.value('@brand[1]',
			'varchar(50)')   
			INNER JOIN	Products.ProdArticle pa
				ON	pa.brand_id = b.brand_id
				AND	pa.sa = ml.value('@saimt[1]',
			'varchar(36)')   
			INNER JOIN	Products.ProdArticleNomenclature pan
				ON	pan.pa_id = pa.pa_id
				AND	pan.sa = ml.value('@sanm[1]',
			'varchar(36)') 
			INNER JOIN Products.TechSize ts
			ON ts.ts_name =  ml.value('@ts[1]',
			'varchar(15)')  OR ts.rus_name =  ml.value('@ts[1]',
			'varchar(15)')  
			INNER JOIN	Products.ProdArticleNomenclatureTechSize pants
				ON	pants.pan_id = pan.pan_id   AND pants.ts_id = ts.ts_id
