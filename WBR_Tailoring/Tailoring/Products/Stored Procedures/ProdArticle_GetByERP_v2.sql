CREATE PROCEDURE [Products].[ProdArticle_GetByERP_v2]
	@pa_id INT
AS
	SET NOCOUNT ON
	...
	FROM	Products.ProdArticle            pa
			...
			OUTER APPLY (
		      		SELECT	ao.erp_id '@id',
		      				CAST(paao.ao_value * ISNULL(si.multiplier, 1) AS DECIMAL(9,2)) '@val',
		      				paao.si_id '@si'
		      		...
		      		WHERE	paao.pa_id = pa.pa_id AND ao.erp_id IS NOT NULL AND ao.isdeleted = 0
