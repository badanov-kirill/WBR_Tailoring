CREATE PROCEDURE [Products].[Sketch_ConstructorLayoutConfirm]
	@sketch_id INT,
	@employee_id INT
AS
	SET NOCOUNT ON
	...
	SELECT	@error_text = CASE 
	      	                   ...
	      	                   WHEN s.ss_id NOT IN (@state_appointed_layout_end) THEN 'Текущий статус ' + ss.ss_name 
	      	                        +
	      	                        ' установленый сотрудником с кодом ' + CAST(s.employee_id AS VARCHAR(10)) 
	      	                        + ' ' + CONVERT(VARCHAR(20), s.dt, 121) +
	      	                        ' не допускает перехода в статус: "Проверена раскладка и взято в работу"'
	      	                   ELSE NULL
	      	              END
	...
	BEGIN TRY
		...
		UPDATE	s
		SET 	ss_id = @state_constructor_take_job_add,
		...
		WHERE	s.sketch_id = @sketch_id
				AND	s.ss_id = @state_appointed_layout_end
