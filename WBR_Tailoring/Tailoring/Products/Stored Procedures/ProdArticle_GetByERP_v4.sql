CREATE PROCEDURE [Products].[ProdArticle_GetByERP_v4]
	@pa_id INT
AS
	SET NOCOUNT ON
	...
	SELECT	pa.pa_id,
			...
			b.erp_id                        brand_id,
			ISNULL(sn.season_id, sn2.season_id) season_id,
			cl.erp_id                       collection_id,
			ISNULL(st.style_id, st2.style_id) style_id,
			ISNULL(d.direction_id, d2.direction_id)     direction_id,
	...
	FROM	Products.ProdArticle            pa
			...
			LEFT JOIN Products.RulesKinds rk
				ON rk.kind_id = k.kind_id
			OUTER APPLY (
