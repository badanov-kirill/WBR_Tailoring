CREATE PROCEDURE [Products].[Sketch_ConstructorToTechDesigner]
	@sketch_id INT,
...
AS
	...
	SELECT	@error_text = CASE 
	      	                   ...
	      	                   WHEN s.ss_id NOT IN (@state_constructor_take_job_add, @state_tech_desig_confirm_from_constructor) THEN 'Текущий статус ' + ss.ss_name 
	      	                        +
	      	                        ' установленый сотрудником с кодом ' + CAST(s.employee_id AS VARCHAR(10)) 
	      	                        + ' ' + CONVERT(VARCHAR(20), s.dt, 121) +
	      	                        ' не допускает перехода в статус: "Тех. эскиз отправлен на доработку конструктором"'
	      	                   ELSE NULL
	      	              END
	...
	BEGIN TRY
		...
		UPDATE	s
		SET 	ss_id = @state_need_tect_desig_correction_from_constructor,
				status_comment = CASE 
		...
		WHERE	s.sketch_id = @sketch_id
				AND	s.ss_id IN (@state_constructor_take_job_add, @state_tech_desig_confirm_from_constructor)
