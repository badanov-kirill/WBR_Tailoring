CREATE PROCEDURE [Planing].[SketchPlanColorVariant_ToCorrectionReserv]
	@spcv_id INT,
	@employee_id INT
AS
	SET NOCOUNT ON
	...
	SELECT	@error_text = CASE 
	      	                   ...
	      	                   WHEN spcv.cvs_id NOT IN (@cv_status_ready, @cv_status_sel_pasp, @cv_status_confectione_end, @cv_status_placing, @cv_status_pre_placing) THEN 
	...
	BEGIN TRY
		...
		SET 	cvs_id = CASE 
		    	              WHEN cvs_id IN (@cv_status_confectione_end, @cv_status_placing, @cv_status_pre_placing) THEN @cv_status_layout_close
		...
		WHERE	spcv_id = @spcv_id
				AND	cvs_id IN (@cv_status_ready, @cv_status_sel_pasp, @cv_status_confectione_end, @cv_status_placing, @cv_status_pre_placing)
