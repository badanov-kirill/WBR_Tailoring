CREATE PROCEDURE [Planing].[SketchPlanColorVariant_SetPrePlacing]
	@sew_office_id INT,
...
AS
	...
	SELECT	@error_text = CASE 
	      	                   ...
	      	                   WHEN spcv.cvs_id NOT IN (@cv_status_confectione_end , @cv_status_add_as_compain, @cv_status_pre_placing)  THEN 'Статус цветоварианта ' + spcv.spcv_name + ' артикула ' + s.sa + ' не позволяет назначить его в цех.'
	...
	WHERE dt.is_mapping = 0 AND
			...
			OR spcv.cvs_id NOT IN (@cv_status_confectione_end , @cv_status_add_as_compain, @cv_status_pre_placing))
