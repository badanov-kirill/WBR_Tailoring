CREATE PROCEDURE [Planing].[SketchPlanColorVariant_Mapping]
	@spcv_id INT,
	@pan_id INT,
	@employee_id INT
AS
	...
	SELECT	@error_text = CASE 
	      	                   ...
	      	                   WHEN spcv.cvs_id NOT IN (@cv_status_ready, @cv_status_sel_pasp_ready, @cv_status_corr_reserv, @cv_status_pasp_get, @cv_status_to_layout, 
	      	                        ...
	      	                        ', операция запрещена.'
	      	                   WHEN spcv.pan_id IS NOT NULL THEN 'Цветовариант ' + spcv.spcv_name + ' уже привязан к артикулу ' + pa.sa + pan.sa
	      	                   ELSE NULL
	      	              END,
			@sp_id = spcv.sp_id
	...
	SELECT	@error_text = CASE 
	      	                   WHEN pan.pan_id IS NULL THEN 'Артикула номенклатуры с кодом ' + CAST(v.pan_id AS VARCHAR(10)) + ' не существует.'
	      	                  -- WHEN pan.nm_id IS NULL THEN 'Артикул ' + pa.sa + pan.sa + ' не сохранен на сайт.'
	      	                   WHEN oa.spcv_name IS NOT NULL THEN 'Артикул ' + pa.sa + pan.sa + ' уже привязан к ' + oa.spcv_name
	...
	BEGIN TRY
		...
		IF @@ROWCOUNT = 0
		BEGIN
		    RAISERROR('Не удалось обновить данные, перечитайте и попробуйте снова', 16, 1)
		    RETURN
		END
	END TRY
	BEGIN CATCH
