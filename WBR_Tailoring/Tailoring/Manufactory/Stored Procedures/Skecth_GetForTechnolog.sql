CREATE PROCEDURE [Manufactory].[Skecth_GetForTechnolog]
	@office_id INT
AS
	SET NOCOUNT ON
	...
	FROM	Manufactory.TaskSew ts   
			...
			LEFT JOIN Manufactory.TaskSewRework tsr
				ON tsr.ts_id = ts.ts_id AND tsr.close_dt IS NULL
	WHERE	ts.is_deleted = 0
			AND	((tss.has_problem_dt IS NOT NULL AND tss.close_problem_dt IS NULL) OR (ts.sew_end_work_dt IS NOT NULL AND tss.close_dt IS NULL)
			OR tsr.tsr_id IS NOT NULL)
			AND	ts.office_id = @office_id	
