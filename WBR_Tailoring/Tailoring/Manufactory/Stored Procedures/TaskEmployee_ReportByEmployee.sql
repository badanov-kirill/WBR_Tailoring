CREATE PROCEDURE [Manufactory].[TaskEmployee_ReportByEmployee]
...
AS
	...
	DECLARE @k1 SMALLINT = 175
	DECLARE @k2 DECIMAL(4,1) = 10.5
	...
	SELECT	ts.pattern_employee_id     employee_id,
			...
			s.pattern_perimeter *6*@k1/88000 work_cost,
			es.employee_name
	...
	FROM	Manufactory.[Sample] s   
			...
			INNER JOIN Products.Brand b
				ON b.brand_id = sk.brand_id
			LEFT JOIN Settings.EmployeeSetting es
				ON ts.cut_employee_id = es.employee_id
	WHERE	ts.cut_end_of_work_dt >= @start_dt
			AND	ts.cut_end_of_work_dt <= @finish_dt
			AND s.cut_perimeter != 0
			AND ts.is_stm = 0
			AND ts.is_deleted = 0
			AND (@employee_id IS NULL OR ts.cut_employee_id = @employee_id)
	UNION ALL	
	...
	FROM	Manufactory.[Sample] s   
			...
			LEFT JOIN Settings.EmployeeSetting es
				ON tsew.sew_employee_id = es.employee_id
	WHERE	tss.close_dt >= @start_dt
			AND	tss.close_dt <= @finish_dt
			AND ts.is_stm = 0
			AND ts.is_deleted = 0
			AND (@employee_id IS NULL OR tsew.sew_employee_id = @employee_id)