CREATE PROCEDURE [Manufactory].[TaskSample_GetBySketchForRM]
	@sketch_id INT
AS
	SET NOCOUNT ON
	...
	WHERE ((ts.cut_begin_work_dt IS NOT NULL AND ts.cut_end_of_work_dt IS NULL) OR oas.sew_employee_id IS NOT NULL)			
			AND	EXISTS (
			   	       	SELECT	1
			   	       	FROM	Manufactory.[Sample] s
			   	       	WHERE	s.task_sample_id = ts.task_sample_id
