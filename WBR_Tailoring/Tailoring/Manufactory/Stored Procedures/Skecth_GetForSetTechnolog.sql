CREATE PROCEDURE [Manufactory].[Skecth_GetForSetTechnolog]
AS
	SET NOCOUNT ON
	...
	WHERE	(
	     		s.technology_employee_id IS NULL
	     		AND (
	     		    	EXISTS (
	     		    		SELECT	1
	     		    		FROM	Manufactory.[Sample] sm   
	     		    				...
	     		    				LEFT JOIN	Manufactory.TaskSewRework tsr
	     		    					ON	tsr.ts_id = tsew.ts_id
	     		    					AND	tsr.close_dt IS NULL
	     		    					ON	tss.sample_id = sm.sample_id
	     		    		WHERE	sm.sketch_id = s.sketch_id
	     		    				AND	(
	     		    				   		(ts.cut_end_of_work_dt IS NOT NULL AND ts.is_deleted = 0 AND sm.is_deleted = 0 AND ts.is_stm = 0 AND sm.sew_launch_dt IS NULL)
	     		    				   		OR (
	     		    				   		   	ts.is_deleted = 0
	     		    				   		   	AND ((tss.has_problem_dt IS NOT NULL AND tss.close_problem_dt IS NULL) OR (tsew.sew_end_work_dt IS NOT NULL AND tss.close_dt IS NULL) OR tsr.tsr_id IS NOT NULL)
	     		    				   		   )
	     		    				   	)
	     		    	)
	     		    	OR oa.create_dt IS NOT NULL
	     		    )
	     	)
	ORDER BY oa.create_dt DESC, s.technology_dt DESC
