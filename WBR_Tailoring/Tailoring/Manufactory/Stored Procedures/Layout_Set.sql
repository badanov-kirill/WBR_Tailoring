CREATE PROCEDURE [Manufactory].[Layout_Set]
	@layout_id INT = NULL,
	@tl_id INT,
	@completing_id INT,
...
AS
	...
	IF ISNULL(@consumption, 0) <= 0
	BEGIN
	    RAISERROR('Не корректный расход.', 16, 1)
	    RETURN
	END
	...
	SELECT	@error_text = CASE 
	      	                   ...
	      	                   WHEN oac.sketch_id IS NULL THEN 'У эскиз с кодом ' + s.sa + ' не заполнена комплектация'
	      	                   WHEN oac.consumption = 0 THEN 'У эскиз с кодом ' + s.sa + ' не заполнен расход в комплектации'
	      	                   ELSE NULL
	      	              END
	FROM	@added_sketch_tab        ast   
			LEFT JOIN	Products.Sketch s
	...
	BEGIN TRY
		...
		BEGIN
		    ...
		    SELECT	v.sketch_id,
		    		...
		    		2
		    FROM	(
		        		SELECT	DISTINCT	laso.sketch_id
		        		FROM	@layout_added_sketch_output laso
		        		WHERE	laso.act != 'D'
		        	) v
		    WHERE NOT EXISTS (
