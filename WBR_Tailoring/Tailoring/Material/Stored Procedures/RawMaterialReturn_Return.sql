CREATE PROCEDURE [Material].[RawMaterialReturn_Return]
	@shkrm_id INT,
	@employee_id INT
AS
	SET NOCOUNT ON
	...
	DECLARE @shkrm_state_dst INT = 17
	DECLARE @proc_id INT
	DECLARE @rmr_id INT
	DECLARE @rmid_id INT
	...
	SELECT	@error_text = CASE 
	      	                   ...
	      	                   WHEN sma.amount != 0 THEN 'Распределена стоимость, возвращать нельзя'
	      	                   WHEN rmr.shkrm_id IS NULL THEN 'На штрихкода ' + CAST(v.shkrm_id AS VARCHAR(10)) + ' не создан документ обмена.'
	      	                   ELSE NULL
	      	              END,
			@rmr_id = rmr.rmr_id,
			@rmid_id = rmr.rmid_id
	...
	BEGIN TRY
		...
		UPDATE	Material.RawMaterialReturn
		SET 	return_dt = @dt,
