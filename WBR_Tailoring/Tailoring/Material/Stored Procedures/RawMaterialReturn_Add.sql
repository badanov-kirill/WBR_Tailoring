CREATE PROCEDURE [Material].[RawMaterialReturn_Add]
	@shkrm_id INT,
	@employee_id INT
AS
	SET NOCOUNT ON
	...
	SELECT	@error_text = CASE 
	      	                   ...
	      	                   WHEN rmr.shkrm_id IS NOT NULL THEN 'Штрихкода ' + CAST(v.shkrm_id AS VARCHAR(10)) + ' уже в документе возврата.'
	      	                   ELSE NULL
	      	              END,
			@rmt_id                         = smai.rmt_id,
			@art_id                         = smai.art_id,
	...
	BEGIN TRY
		...
		INSERT INTO Material.RawMaterialReturn
		  (
		    doc_id,
		    doc_type_id,
		    create_dt,
