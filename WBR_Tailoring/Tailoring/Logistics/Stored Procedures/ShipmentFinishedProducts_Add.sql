CREATE PROCEDURE [Logistics].[ShipmentFinishedProducts_Add]
	@employee_id INT
AS
	SET NOCOUNT ON
	...
	BEGIN
	    RAISERROR('Уже есть неотправленная отгрузка из этого офиса. Нельзя собирать две отгрузки готовой продукции одновременно', 16, 1)
	    RETURN
	END
