CREATE PROCEDURE [Logistics].[PackingBoxDetail_ToBox_v3]
...
AS
	...
	SELECT	@error_text = CASE 
	      	                   ...
	      	                   WHEN oabc.barcode1 IS NOT NULL THEN 'После перемещения в коробке окажется один артикул ' + pa.sa + pan.sa + ' с разными баркодами ' + oabc.barcode1 + ' и ' + dt.barcode
	      	                   ELSE NULL
	      	              END
	FROM	(VALUES(@pants_id, @barcode))dt(pants, barcode)   
			...
			OUTER APPLY (
			      	SELECT	MAX(pbd.barcode) barcode1
			      	FROM	Manufactory.ProductUnicCode puc   
			      			INNER JOIN	Logistics.PackingBoxDetail pbd
			      				ON	pbd.product_unic_code = puc.product_unic_code
			      	WHERE	puc.pants_id = pants.pants_id 
			      			AND	pbd.packing_box_id =  @dst_packing_box_id
			      			AND pbd.barcode != dt.barcode
			      ) oabc
	...
	FROM	Products.ProdArticleNomenclatureTechSize pants   
			...
			INNER JOIN	Logistics.PackingBoxDetail pbd
				ON	pbd.product_unic_code = puc.product_unic_code
				AND	pbd.packing_box_id = @src_packing_box_id
	WHERE	pants.pants_id = @pants_id AND pbd.barcode = @barcode
	...
	SELECT @error_text = 'После переноса в коробке ' + CAST(vg.packing_box_id AS VARCHAR(10)) +
